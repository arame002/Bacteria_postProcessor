...
def plot_Trajectory_Panel(particle_data, particle,dir_name ) :
...
standingVelocity_Threshold = 5.0 # in µm/s
standingDuration_Threshold = 10 # in frames
...
for index in range(1, numInputFiles + 1):
    ...
    for particle, particle_data in particles.items():
        ...
        if len(x)>0 and find_max_pairwise_distance(x,y,pairwiseDist_Threshold) and generateTrajectories == True :
            plot_Trajectory_Panel(particle_data, particle, dir_name )
    plt.close('all')
    ...
    for particle in particles:
        ...
        for i in range(len(events)-1):
            ...
            tmpRunV = sum( tmpV[events[i]:events[i+1] ] )/ duration
            if duration > filterShortReverses / temporalScaling :
                durations.append(duration)
...
for particle, particle_data in all_filteredParticles.items():
    if generateTrajectories == True :
        plot_Trajectory_Panel(particle_data, particle, directory)
plt.close('all')
